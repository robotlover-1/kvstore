...
if [[ "$SAVE_OUT" != "-ERR save failed" ]]; then
  echo "[FAIL] SAVE should fail in persist_mode=none, got: $SAVE_OUT" >&2
  exit 1
fi

CFG_OUT="$(send_resp '*3\r\n$6\r\nCONFIG\r\n$10\r\nAPPENDFSYNC\r\n$7\r\nalways\r\n' | tr -d '\r')"
if [[ "$CFG_OUT" != "-ERR invalid appendfsync" ]]; then
  echo "[FAIL] CONFIG APPENDFSYNC should fail when aof disabled, got: $CFG_OUT" >&2
  exit 1
fi
