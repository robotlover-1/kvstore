...
| **custom** | 基线 | 0 | 5,548 | 3,812 | 68.7% |
| | 写入 1% | 1w | 7,440 | 5,056 | 68.0% |
| | 写入 10% | 10w | 15,632 | 13,896 | 88.9% |
| | 写入 50% | 50w | 51,536 | 49,800 | 96.6% |
| | **写入 100%（峰值）** | **100w** | **96,592** | **94,856** | **98.2%** |
| | 释放 50% | 50w | 56,464 | 54,728 | 96.9% |
| | 释放 80% | 20w | 30,992 | 29,256 | 94.4% |
| | 释放 100% | 0 | **14,032** | **12,296** | **87.6%** |
...
- **custom**：唯一同时归还虚拟内存的后端——munmap 整页时 VmSize 随 VmRSS 同步下降（96,592 → 14,032 KB）。释放率最高（87%），但呈阶梯状——必须整页完全空闲才触发回收。
