...
**为什么小数据量波动大？** 所有请求都是 INSERT（`-r $data_size` 保证 key 不重复），hash 表从空开始不断扩容：

| 数据量 | hash 表扩容路径 | rehash 次数 | rehash 耗时占比 |
|--------|---------------|-----------|---------------|
| 1k | 16→32→64→128→256→512→1024 | 6 次 | **高**（每次阻塞事件循环，1000 请求的 fsync 被 rehash 打断） |
| 1w | 16→...→16384 | ~10 次 | 中（前几千条在扩容，后半段稳态） |
| 10w | 16→...→131072 | ~13 次 | 低（99% 的时间在稳态） |
| 100w | 16→...→1M+ | ~16 次 | **可忽略**（扩容在 8 秒中的前 0.1 秒完成） |

1k 每次 rehash 的时机稍有不同（取决于随机 key 的分布），就会显著影响那 1000 次 fsync 的完成节奏，导致 QPS 在 14K–42K 之间剧烈波动。100w 不受影响因为 99.9% 的请求都在稳态。
