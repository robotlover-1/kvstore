...
| **custom** | 基线 | 0 | 5,528 | 3,876 | 70.1% |
| | 写入 1% | 1w | 7,420 | 5,120 | 69.0% |
| | 写入 10% | 10w | 14,908 | 13,256 | 88.9% |
| | 写入 50% | 50w | 47,676 | 46,024 | 96.5% |
| | **写入 100%（峰值）** | **100w** | **88,764** | **87,112** | **98.1%** |
| | 释放 50% | 50w | 52,604 | 50,952 | 96.9% |
| | 释放 80% | 20w | 29,436 | 27,784 | 94.4% |
| | 释放 100% | 0 | **13,884** | **12,232** | **88.1%** |

##### 结果分析

**① 峰值内存效率：libc 最优（73 MB），custom 居中（89 MB），jemalloc 最高（96 MB）**

libc（ptmalloc2）使用 sbrk + mmap 混合策略，arena 内碎片较少，100w key 仅需 73 MB 虚拟内存（~75 bytes/条目）。custom 经过 17 级 slab class 优化（~1.25× 间距，内部碎片 ≤25%）后降至 89 MB（~91 bytes/条目），比优化前减少 8%。jemalloc 基于 mmap 页面粒度分配，虚拟内存偏高（~99 bytes/条目）。
...
| **custom** | 87,112 KB | 12,232 KB | **86.0%** |
