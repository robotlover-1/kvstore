...
客户端:  [写数据] ─────────── [再写5w] ────────────────── [继续写...]
         ↓                    ↓                            ↓
eBPF:    [tcp_recvmsg探测] → [缓存到ringbuf] → [探测REPLDONE]
                                                 → [flush缓存+增量同步]
```
...
| **②** | 客户发送给主机的数据（增量同步用） | `kprobe/tcp_recvmsg`（新增 BPF） | 捕获客户端写入命令，用于增量同步转发 |
| **③** | 缓存全量同步时客户发送给主机的数据 | 与②共用 `tcp_recvmsg` hook | 全量同步期间缓存客户端数据，完成后一起转发 |
...
│            │  repl_broadcast  │                                         │
│            │ ┌──────────────┐│                                         │
│            │ │ backlog_feed ││ ← 全量同步期间仅进 backlog               │
│            │ └──────────────┘│                                         │
│            │ ┌──────────────┐│                                         │
│            │ │realtime_send ││ ← 全量同步完成后发送到 slave             │
│            │ └──────┬───────┘│                                         │
│            └────────┼────────┘                                         │
│                     │                                                   │
│                     │ ① tcp_sendmsg hook (master→slave)                │
│                     ▼                                                   │
│  ┌──────────────────────────┐                                         │
│  │ Slave TCP 连接           │                                          │
│  │ → FULLRESYNC header      │  ← RDMA 全量数据                         │
│  │ → snapshot data          │                                          │
│  │ → REPLDONE               │  ← eBPF 探测到后触发 flush               │
│  │ → flush缓存数据          │  ← eBPF ringbuf 中的缓存数据             │
│  │ → backlog gap replay     │  ← backlog 中的 gap 数据                 │
│  │ → 后续增量数据           │  ← 正常的增量同步                         │
│  └──────────────────────────┘                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| # | 文件 | 改动内容 |
|---|------|---------|
| 1 | `src/main/kvstore.c` | 移除 `main()` 中 `start_rdma_master_listener()`；`queue_snapshot()` 整合 RDMA start/stop + flush；`repl_broadcast()` 增加全量同步跳过逻辑；新增全局标志 `g_repl_fullsync_in_progress`；`main()` 中新增 `repl_client_capture_init()` |
| 2 | `src/replication/kvs_repl.c` | 新增 `repl_rdma_start_fullsync()` / `repl_rdma_stop_fullsync()`；从机 `slave_thread()` 改造建连流程 |
| 3 | `src/replication/kvs_repl_kprobe.c` | 新增 REPLDONE 回调处理；新增 client capture 加载/管理/缓存队列/flush 逻辑；新增统计 |
...
| 5 | `include/kvstore/kvstore.h` | 新增 `g_repl_fullsync_in_progress` 外部变量声明；新增函数声明 |
