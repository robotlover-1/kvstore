...
##### 内存占用测试（2026-07-13 重测，含三项内存优化）

> 测试脚本：`python3 tools/bench/mem_pool_bench.py`
>
> **测试方法**：启动 kvstore → 写入 100w 条 HSET → 释放 100w 条 HDEL，在 1%/10%/50%/80%/100% 进度点采样 `/proc/<pid>/status` 的 VmSize（虚拟内存）和 VmRSS（物理内存）。AOF 关闭，Hash 引擎，REPL 传输设为 TCP（避免 RDMA/BPF 干扰）。
>
...
>
> **jemalloc 配置**：`MALLOC_CONF="dirty_decay_ms:0,muzzy_decay_ms:0,background_thread:true,retain:false"` ——启用后台线程驱动 decay，且允许 munmap 归还虚拟地址空间。


| 后端         | 阶段                  | 数据量   | VmSize (KB) | VmRSS (KB) | RSS/Size  |
| ------------ | --------------------- | -------- | ----------- | ---------- | --------- |
| **libc**     | 基线                  | 0        | 5,636       | 3,356      | 59.5%     |
|              | 写入 1%               | 1w       | 6,536       | 4,144      | 63.4%     |
|              | 写入 10%              | 10w      | 12,696      | 10,476     | 82.5%     |
|              | 写入 50%              | 50w      | 39,232      | 37,012     | 94.3%     |
|              | **写入 100%（峰值）** | **100w** | **72,400**  | **70,312** | **97.1%** |
|              | 释放 50%              | 50w      | 72,400      | 39,084     | 54.0%     |
|              | 释放 80%              | 20w      | 72,400      | 20,332     | 28.1%     |
|              | 释放 100%             | 0        | 7,144       | 3,764      | 52.7%     |
| **jemalloc** | 基线                  | 0        | 30,844      | 5,520      | 17.9%     |
|              | 写入 1%               | 1w       | 31,272      | 6,064      | 19.4%     |
|              | 写入 10%              | 10w      | 35,908      | 10,752     | 29.9%     |
|              | 写入 50%              | 50w      | 62,432      | 38,120     | 61.1%     |
|              | 写入 80%              | 80w      | 83,232      | 59,540     | 71.5%     |
|              | **写入 100%（峰值）** | **100w** | **95,732**  | **72,456** | **75.7%** |
|              | 释放 50%              | 50w      | 66,060      | 42,788     | 64.8%     |
|              | 释放 80%              | 20w      | 47,312      | 24,040     | 50.8%     |
|              | 释放 100%             | 0        | **30,748**  | **7,476**  | **24.3%** |
| **custom**   | 基线                  | 0        | 5,848       | 3,632      | 62.1%     |
|              | 写入 1%               | 1w       | 6,456       | 4,264      | 66.0%     |
|              | 写入 10%              | 10w      | 12,284      | 10,140     | 82.5%     |
|              | 写入 50%              | 50w      | 38,036      | 35,904     | 94.4%     |
|              | 写入 80%              | 80w      | 58,344      | 56,160     | 96.3%     |
|              | **写入 100%（峰值）** | **100w** | **70,452**  | **68,276** | **96.9%** |
|              | 释放 1%               | 99w      | 70,004      | 67,828     | 96.9%     |
|              | 释放 10%              | 90w      | 64,756      | 62,580     | 96.6%     |
|              | 释放 50%              | 50w      | 41,332      | 39,156     | 94.7%     |
|              | 释放 80%              | 20w      | 23,732      | 21,556     | 90.8%     |
|              | 释放 100%             | 0        | **7,956**   | **5,780**  | **72.6%** |

##### 结果分析

**① 峰值内存效率：custom 最优（70.5 MB），libc 居中（72.4 MB），jemalloc 最高（95.7 MB，因 retain:false 仍保留一定虚拟地址空间）**

**② 物理内存释放率（VmRSS）：libc 最优（99.4%），custom 和 jemalloc 均 ~97%**
...
- **custom**：峰值最低，VmSize/VmRSS 同步收缩至基线附近（+2 MB），hash 缩容 + 无 backlog 双作用。
