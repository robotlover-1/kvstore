...
static int apply_config_kv(const char *key, const char *val) {
    ...
    else if (!strcmp(key, "persist_mode")) snprintf(g_cfg.persist_mode, sizeof(g_cfg.persist_mode), "%s", val);
...
static int parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (!strcmp(argv[i], "--bind") && i + 1 < argc) snprintf(g_cfg.bind_ip, sizeof(g_cfg.bind_ip), "%s", argv[++i]);
            ...
            snprintf(g_cfg.persist_mode, sizeof(g_cfg.persist_mode), "%s", argv[++i]);
        }
...
int handle_parsed_command(conn_t *c, int argc, char **argv, size_t *argl, const unsigned char *raw, size_t rawlen, int from_replication) {
    ...
    if (rc == 0) {
        ...
        n = resp_error(resp, BUFFER_CAP, "not found or exists");
    } else {
