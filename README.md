...
> **2026-06-26 重测**（parse_resp_stream 还原 copy 方式后）

| 配置 | ECHO (QPS) | HSET (QPS) |
|---|---|---|
| **kvstore** (ECHO 基线) | **132,223** | — |
| ├─ AOF 关闭（`--aof-disable`） | — | **80,502** |
| ├─ AOF always（每条命令 fsync） | — | **77,869** |
| └─ AOF everysec（每秒 fsync） | — | **134,535** |
...
**① ECHO 基线——kvstore RESP 解析比 Redis 快 11.1%**（132,223 vs 119,062 QPS）

**② kvstore AOF always 开销极小**
AOF always (77,869) vs AOF 关闭 (80,502) 仅差 **3.3%**，Group Commit + io_uring 批量 fsync 将持久化开销几乎完全隐藏。

**③ HSET 并发下 Redis 仍领先**（AOF 关闭：80,502 vs 124,285），差距来自 hash 引擎的渐进式 rehash 在高并发写入时的扩容开销。
...
|---|---|---|---|
| kvstore AOF disable | 80,502 | 39,438 | 2.0× |
| kvstore AOF always | 77,869 | 53,019 | 1.5× |
| kvstore AOF everysec | 134,535 | 39,981 | 3.4× |
